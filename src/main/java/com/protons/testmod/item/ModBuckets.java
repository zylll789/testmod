...
public class ModBuckets extends BucketItem {
    private final Fluid fluid;

    public ModBuckets(Fluid fluid, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (blockHitResult.getType() == HitResult.Type.MISS) {
            ...
            if (!world.canPlayerModifyAt(user, blockPos) || !user.canPlaceOn(blockPos2, direction, itemStack)) {
                ...
                BlockState blockState = world.getBlockState(blockPos);
                BlockPos blockPos3 = blockState.getBlock() instanceof FluidFillable && this.fluid instanceof ModFluid ? blockPos : blockPos2;
    ...
    public boolean placeFluid(@Nullable PlayerEntity player, World world, BlockPos pos, @Nullable BlockHitResult hitResult) {
        if (!(this.fluid instanceof FlowableFluid flowableFluid)) {
            ...
            if (!bl2) {
                ...
                if (block instanceof FluidFillable fluidFillable && this.fluid instanceof ModFluid) {
