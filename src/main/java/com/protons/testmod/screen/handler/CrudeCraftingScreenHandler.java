package com.protons.testmod.screen.handler;
...
public class CrudeCraftingScreenHandler extends AbstractRecipeScreenHandler<RecipeInputInventory> {
    ...
    private final RecipeInputInventory input = new CrudeCraftingInventory(this, 3, 3);
    private final CraftingResultInventory result = new CraftingResultInventory();
    private final ScreenHandlerContext context;
    private final PlayerEntity player;

    public CrudeCraftingScreenHandler(int syncId, PlayerInventory playerInventory) {
        this(syncId, playerInventory, ScreenHandlerContext.EMPTY);
    }

    public CrudeCraftingScreenHandler(int syncId, PlayerInventory playerInventory, ScreenHandlerContext context) {
        super(ModScreens.CRUDE_CRAFTING, syncId);
        this.context = context;
        this.player = playerInventory.player;
        this.addSlot(new CraftingResultSlot(playerInventory.player, this.input, this.result, 0, 134, 28));
    ...
    public ItemStack quickMove(PlayerEntity player, int slot) {
        ...
        if (slot2 != null && slot2.hasStack()) {
            ...
            } else if (slot >= 8 && slot < 44) {
                if (!this.insertItem(itemStack2, 1, 8, false)) {
                    if (slot < 35) {
                        if (!this.insertItem(itemStack2, 35, 44, false)) {
                            return ItemStack.EMPTY;
                        }
                    } else if (!this.insertItem(itemStack2, 8, 35, false)) {
                        return ItemStack.EMPTY;
                    }
                }
    ...
    public boolean canUse(PlayerEntity player) {
        return canUse(this.context, player, ModBlocks.CRUDE_CRAFTING_TABLE);
    }
